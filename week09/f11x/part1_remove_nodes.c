// F11X Tutorial 09 - part1_remove_nodes.c
//
// Program to demonstrate removing nodes from a linked list
...
struct node *remove_head(struct node *head) {
    ...
    head = head->next;
    // Free the memory allocated for the old head
    free(old);
...
struct node *remove_tail(struct node *head) {
    ...
    // Free the last node
    free(current->next);
    // Update the second last node to point to NULL (i.e. becomes last node)
    current->next = NULL;
    return head;
}